...
function checkLoginStatus() {
  ...
  if (isLoggedIn) {
    // 로그인 상태일 때 수행할 동작
    showLoggedInUI(reviewFormContainer);
  } else {
    // 로그아웃 상태일 때 수행할 동작
    showLoggedOutUI(reviewFormContainer);
  }
}
...
const signupFormContainer = document.getElementById('signupFormContainer');
 ...
 document.getElementById("commentForm").addEventListener("submit", function (event) {
  ...
  if (isLoggedIn) {
    // 로그인 상태에서는 비밀번호 칸을 제외하고 사용자명과 댓글 내용만 가져옴
 ...
 function checkLoginStatus() {
  if (isLoggedIn) {
    // 로그인 상태일 경우
    document.getElementById("loginForm").style.display = "none"; // 로그인 폼 숨김
    document.getElementById("commentForm").style.display = "block"; // 댓글 작성 폼 표시
  } else {
    // 비로그인 상태일 경우
    document.getElementById("loginForm").style.display = "block"; // 로그인 폼 표시
    document.getElementById("commentForm").style.display = "none"; // 댓글 작성 폼 숨김
  }
}
